...
export const probeStorageRegisterStorageDevice: EndpointProbe = {
  ...
  async run(ctx: ProbeContext): Promise<EndpointDoc> {
    ...
    try {
      ...
      const form = { deviceId };
      const response = await probePostForm("/storage/register-storage-device", form, session.token);
      ...
      return {
        ...
        summary: "Register storage device",
        method: "POST",
        path: "/storage/register-storage-device",
        phase: "async-command",
        auth: "authorization-token",
        parameters: [{ in: "form", name: "deviceId", required: true, type: "string" }],
        responses: [{ status: 202, description: "Promotion accepted." }],
        examples: [{
          ...recordExample({ name: "Happy path", method: "POST", path: "/storage/register-storage-device", bodyType: "form", body: form, response }),
